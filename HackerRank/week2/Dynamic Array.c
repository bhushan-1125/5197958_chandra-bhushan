...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
     // Allocate an array of int* for the sequences
    int **seq = malloc(n * sizeof(int*));
    int *sizes = calloc(n, sizeof(int));       // sizes of each sequence
    int *capacity = calloc(n, sizeof(int));    // capacity for each sequence

    // Initially, all sequences are empty
    for (int i = 0; i < n; i++) {
        seq[i] = NULL;
        capacity[i] = 0;
    }
