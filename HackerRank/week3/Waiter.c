 ...
 *
 */
// Helper: generate first k primes
void generate_primes(int k, int *primes) {
    int count = 0, num = 2;
    while (count < k) {
        int is_prime = 1;
...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    // We'll simulate stacks using arrays
    ...
    for (int i = 0; i < q; i++) {
        ...
        // Process A stack (from top = end to bottom = start)
        ...
        // Output B in order (top to bottom = last pushed first)
        for (int j = B_size - 1; j >= 0; j--) {
            result[res_idx++] = B[j];
        }

        // Prepare A for next round
        free(A);
        A = nextA;
        A_size = nextA_size;
    ...
    // Finally, dump remaining A (in stack order)
    for (int j = A_size - 1; j >= 0; j--) {
        result[res_idx++] = A[j];
    }
