 ...
 *
 */
char** copy_grid(int rows, int cols, char** src) {
...
char** explode(int rows, int cols, char** grid) {
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    *result_count = grid_count;
    ...
    if (n == 1) {
        return copy_grid(grid_count, cols, grid);
    }
    if (n % 2 == 0) {
        return full_bombs(grid_count, cols);
    }

    char** first = explode(grid_count, cols, grid);
    if (n % 4 == 3) {
        return first;
    }
    char** second = explode(grid_count, cols, first);

    // free first explosion grid
